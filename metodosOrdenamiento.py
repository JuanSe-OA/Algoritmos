...
def heap_sort(arr):
    heap = []
    for item in arr:
        anio = obtener_anio_valido(item)
        autor = item.get('autor', '')  # Si 'autor' no está, usa una cadena vacía
        heapq.heappush(heap, (anio, autor, item))  # Insertamos una tupla (año, autor, diccionario)
    
    sorted_list = []
    while heap:
        _, _, articulo = heapq.heappop(heap)  # Extraemos los valores ignorando los primeros dos elementos
        sorted_list.append(articulo)  # Agregamos el diccionario ordenado a la lista final
...
# Clase Node definida fuera de la función tree_sort
class Node:
...
# Función insert que inserta un nodo en el árbol
def insert(root, key):
    if root is None:
        return Node(key)
...
# Función inorder_traversal para recorrer el árbol inorden
...
# Función tree_sort que usa los métodos anteriores
def tree_sort(arr):
    if not arr:
        return arr
    root = None
...
def main(folder_path, num_articles):
    ...
    print(f"Procesamiento completado con {n} artículos ordenados.")
